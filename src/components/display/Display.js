...
export default class Display extends React.Component {
  ...
  focusBelowApp = (curAppID) => {
    const openedApps = [...this.state.openedApps]
    const curApp = openedApps.find((app) => app.id === curAppID)
    if (openedApps.length < 2 || !curApp) return false
    let belowApp = null
    openedApps.forEach((app) => {
      if (!belowApp || (app.zIndex < curApp.zIndex && app.zIndex > belowApp.zIndex)) belowApp = app
    })
